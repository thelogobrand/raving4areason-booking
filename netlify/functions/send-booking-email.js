export async function handler(event) {
  try {
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body:JSON.stringify({from:`Raving 4 A Reason <${emailSettings.from_email || "onboarding@resend.dev"}>`,to:[booking.parent_email,emailSettings.notification_email || "unitypromotionsuk@gmail.com"],reply_to:emailSettings.reply_to_email || undefined,subject:`R4AR Booking Confirmation${booking.booking_ref ? ` — ${safe(booking.booking_ref)}` : ""} — ${safe(booking.date)} ${safe(booking.time)}`,html})
    });
