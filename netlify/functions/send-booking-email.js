export async function handler(event) {
  try {
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body:JSON.stringify({from:"Raving 4 A Reason <onboarding@resend.dev>",to:[booking.parent_email,"unitypromotionsuk@gmail.com"],subject:`R4AR Booking Confirmation — ${safe(booking.date)} ${safe(booking.time)}`,html})
    });
